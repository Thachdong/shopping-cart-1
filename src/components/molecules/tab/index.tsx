...
const ACTIVE_PANEL_ITEM_CLASSNAME = "border-primary-100 bg-gray-200 text-primary-100 font-bold";
...
export const Tab: React.FC<Readonly<TTab>> = ({
  ...
  const tabPanel = useMemo(() => {
    return items.map((item) => {
      const activeClass = item.id === activeTab ? ACTIVE_PANEL_ITEM_CLASSNAME : "";

      return (
        <li className={joinClass(DEFAULT_PANEL_ITEM_CLASSNAME, activeClass)} key={item.id} onClick={() => activateTab(item.id)}>
          {item.label}
        </li>
      );
    });
